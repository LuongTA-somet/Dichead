(function () {
    ...
    if (window.document) {
        ...
        // Font
        var loadFont = (item, callback) => {
            var ttfIndex = item.url.lastIndexOf(".ttf");

            var slashPos = item.url.lastIndexOf("/");
            var fontFamilyName;
            if (slashPos === -1) {
                fontFamilyName = item.url.substring(0, ttfIndex) + "_LABEL";
            } else {
                fontFamilyName = item.url.substring(slashPos + 1, ttfIndex) + "_LABEL";
            }
            ...
            data = data.replace("PASTE-BASE64-HERE", window.resMap[item.url]);
